...
### G-d. 리뷰는 "어느 영화에 붙어도 자연스러운 것"만 고른다

임의 배정을 그대로 두니 기생충 리뷰에 이런 것들이 붙었다.
...
명백히 다른 작품 이야기다. 임의 배정이라고 **문서에 적어 두더라도, 화면을 보는 사람은
먼저 "데이터가 깨졌나"로 읽는다.** 설명은 그 인상보다 늦게 도착한다.

→ 시드 선별에 **일반 평가문 필터**를 넣었다.
...
| 중복 제거 | NSMC에는 같은 문장이 다른 id로 여러 번 있어 서로 다른 영화에 중복 배정된다 |

희귀 어절 필터가 핵심이다. `사이먼래틀` · `엄석대` · `하이바라` 같은 고유명사는
접미사 규칙으로 잡히지 않지만 **말뭉치에서 드물다**는 공통점이 있다.
형태소 분석기 없이 표준 라이브러리만으로 처리했다.

15만 건에서 후보 약 1,800건이 남았고, 그중 72건을 골랐다.
빈도 필터를 통과할 만큼 흔한 배우 이름(`견자단` 등)은 일반화가 불가능해 목록으로 따로 뺐다.
...
콜드 스타트 2.64초는 **모델 로드가 대부분**이다. `--min-instances=0`이라
유휴 상태에서 인스턴스가 내려가므로, 화면에서는 첫 요청이 느릴 수 있다는 전제로
타임아웃을 넉넉히 잡고 오류 배너를 준비해 두었다.

### H-d. 인스턴스 교체 시 데이터 초기화 — 배포본 실증

로컬 컨테이너에서 확인한 제약이 실제 Cloud Run에서도 그대로 재현되는지 확인했다.
...
배포하고 나니 **누구나 시드 영화를 삭제할 수 있고, 리뷰 12건이 함께 사라지는** 상태였다.
기존 방어는 화면의 2단계 확인(체크박스)뿐인데 이건 실수만 막지 고의를 막지 못한다.
"인스턴스가 교체되면 시드로 복원된다"는 성질도 방어가 아니다 — **언제 일어날지 제어할 수 없어서**,
하필 심사 중에 화면이 비어 있으면 의미가 없다.

그렇다고 삭제 기능을 없앨 수는 없다. **영화 삭제는 가이드의 필수 기능**이다.
...
- `PROTECT_SEED` 환경변수로 켜고 끈다. **배포본에서만 켠다.**
- 시드 영화의 **리뷰 삭제도 함께 막는다.** 영화만 막으면 리뷰를 하나씩 지워
  같은 결과(빈 화면)를 만들 수 있다.
- 삭제 가능 여부는 **서버가 판단해 `deletable` 필드로 내려준다.** 화면이 스스로
  규칙을 갖지 않게 했다 — 규칙이 두 곳에 있으면 언젠가 어긋난다.
- 필수 기능 시연은 그대로 가능하다. 영화를 하나 등록한 뒤 지우면 된다.

시드 영화의 위험 구역은 아래처럼 삭제 버튼이 비활성화되고 이유가 함께 표시된다.
...
4. **SQLite WAL 때문에 이미지에 옛 DB가 들어갔다.** 시드 보호 기능을 넣고 배포했더니
   `no such column: movie.is_seed`로 500이 났다. 로컬에서는 멀쩡했는데,
   앱이 연결마다 `PRAGMA journal_mode=WAL`을 걸기 때문에 스키마 변경이
   `movies.db-wal`에만 남아 있었고 `.dockerignore`가 그 파일을 제외한 것이다.
   **`movies.db` 하나만 복사되면서 변경분이 통째로 사라졌다.**
   → 시드 스크립트 마지막에 `wal_checkpoint(TRUNCATE)` + `journal_mode=DELETE`를
   수행해 **DB가 단일 파일로 완결되게** 만들었다. 저널 모드 전환에는 배타적 잠금이
   필요하므로 세션을 모두 닫은 뒤 전용 연결로 실행한다.
...
| 공개 배포본 삭제 | 시드만 보호 | 필수 기능(삭제) 시연과 데모 보존을 동시에 만족한다 |
